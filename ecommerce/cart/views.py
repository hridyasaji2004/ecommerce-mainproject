...
#         stock = checkstock(c)
#
#         form_instance = OrderForm()  # Create form instance outside the if condition
#         context = {'form': form_instance}  # Always include form in context
#
#         if stock:
#             return render(request, 'checkout.html', context)
#         else:
#             messages.error(request, "Currently items not available, can't place order.")
#             return render(request, 'checkout.html', context)  # Pass context here too
#
#     def post(self, request):
#         form_instance = OrderForm(request.POST)
#         if form_instance.is_valid():
